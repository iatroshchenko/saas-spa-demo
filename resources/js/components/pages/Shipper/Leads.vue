              ...
              <div class="aside-content">
                <div class="aside-header">
                  <div class="customer-header d-flex align-items-center justify-content-between">
                    ...
                    <a @click="onCreateModalOpen" data-toggle="modal" data-target="#createModal" class="btn btn-primary mr-2 mb-2 mb-md-0">Create Lead</a>
                  </div>
                </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loadClients() {
        return client.get('/api/shipper/' + this.$route.params.shipper_id + '/leads')
          .then(res => {
      ...
      createUser() {
        this.clearCreateErrors();
        client.post('/api/shipper/' + this.$route.params.shipper_id + '/leads/', {
          ...this.createFormData
        })
          .then(res => {
            this.$swal('Success', 'Lead has been created', 'success')
              .then(() => {
                this.loadClients()
                  .then(res => {
      ...
      saveUser() {
        ...
        client.post('/api/shipper/' + this.$route.params.shipper_id + '/clients/' + this.editFormData.id, {
          ...
          .then(res => {
            this.$swal('Success', 'Lead has been updated', 'success')
              .then(() => {
                this.loadClients()
                  .then(res => {
      ...
      deleteUser(user){
        client.post('/api/shipper/' + this.$route.params.shipper_id + '/users/' + user.id + '/delete')
          .then(res => {
            this.$swal('Success', 'Lead has been successfully deleted', 'success')
              .then(() => {
