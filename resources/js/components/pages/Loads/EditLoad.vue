<template>
  <div class="row">
    <div v-if="load" class="col-md-12 grid-margin stretch-card">
      <div class="card">
        <div class="card-body">
          <h6 class="card-title">Edit Load</h6>
          ...
          <div class="row">
                  ...
                  <div class="card">
                    <div class="card-body">
                      ...
                      <div class="forms-sample">
                        ...
                        <div v-if="user && !['Client', 'Driver'].includes(user.role)" class="form-group">
                        ...
                        <div class="form-group">
                          ...
                          <p v-if="currentAttachments && currentAttachments.length" class="card-description">
                            Maximum file size is 2M. Current Attachments:
                          </p>
                          <p v-else class="card-description">
                            Maximum file size is 2M
                          </p>
                          <div v-if="currentAttachments && currentAttachments.length" class="current-attachments">
                            <div v-for="a in currentAttachments" class="current-attachment-links">
                  ...
                  <div class="card">
                    <div class="card-body">
                      ...
                      <div class="form-group">
                        ...
                        <select v-if="carrierTypes" v-model="formData.carrier_type" class="form-control" id="carrier_type">
                          <option selected="" disabled="">Select Carrier Type</option>
                          <option :key="key" v-for="(item, key) in carrierTypes" :value="item.key">
                            {{ item.value }}
                          </option>
                        </select>
                      </div>

                      <div :key="key" v-for="(error, key) in errors.carrier_type" class="form-group validation-error">
                        {{ error }}
                      </div>
                    ...
                    <div class="card-body">
                      ...
                      <div class="form-group has-predictions">
                        ...
                        <div v-if="predictions.origin.zip && predictions.origin.zip.length" class="predictions">
                          <div @click="fillOriginInfoByZipPrediction(prediction)" v-for="prediction in predictions.origin.zip" class="predictions-prediction p-2">
                            {{ prediction.description }}
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="form-group has-predictions">
                        ...
                        <div v-if="predictions.origin.city && predictions.origin.city.length" class="predictions">
                          <div @click="fillOriginInfoByCityPrediction(prediction)" v-for="prediction in predictions.origin.city" class="predictions-prediction p-2">
                            {{ prediction.description }}
                          </div>
                        </div>
                      </div>
                    ...
                    <div class="card-body">
                      ...
                      <div class="form-group has-predictions">
                        ...
                        <input
                          ...
                          type="text"
                          class="form-control"
                          id="load_destination_zip" autocomplete="chrome-off" placeholder="Zip">

                        <div v-if="predictions.destination.zip && predictions.destination.zip.length" class="predictions">
                          <div @click="fillDestinationInfoByZipPrediction(prediction)" v-for="prediction in predictions.destination.zip" class="predictions-prediction p-2">
                            {{ prediction.description }}
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="form-group has-predictions">
                        ...
                        <div v-if="predictions.destination.city && predictions.destination.city.length" class="predictions">
                          <div @click="fillDestinationInfoByCityPrediction(prediction)" v-for="prediction in predictions.destination.city" class="predictions-prediction p-2">
                            {{ prediction.description }}
                          </div>
                        </div>
                      </div>
                ...
                <div class="col-12 grid-margin stretch-card">
                  <div class="card">
                    <div class="card-body">
                      ...
                      <div v-if="errors.load_vehicles && errors.load_vehicles.length && !Array.isArray(errors.load_vehicles[0])">
                          ...
                          <div class="row">
                            <div class="col-12 col-lg-6">
                              ...
                              <div class="form-group form-check">
                                <label class="form-check-label">
                                  <input v-model="formData.load_vehicles[key].is_running" type="checkbox" class="form-check-input">
                                  Running
                              ...
                              <div v-if="errors.load_vehicles[key]">
                                <div v-for="error in errors.load_vehicles[key].is_running" class="form-group validation-error">
                                  {{ error }}
                                </div>
                              </div>
                              ...
                              <div class="form-group">
                                ...
                                <select v-if="carSizes" v-model="formData.load_vehicles[key].size" class="form-control" :id="'vehicles_' + key + '_' + 'size'">
                              ...
                              <div v-if="errors.load_vehicles[key]">
                                <div v-if="errors.load_vehicles[key]" v-for="error in errors.load_vehicles[key].size" class="form-group validation-error">
                                  {{ error }}
                                </div>
                              </div>
            ...
            <div class="col-12">
              <div @click="updateLoad">
                <a class="btn btn-primary">Update load</a>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        load: null,
        updatingLoad: null, // loading flag when send update request
        ...
        formData: {
          ...
          load_origin_zip: null,
          load_origin_city: null,
          load_origin_state: null,
          ...
          load_destination_zip: null,
          load_destination_state: null,
          load_destination_city: null,
        ...
        errors: {
          ...
          // load destination optional fields
          load_destination_name: [],
          load_destination_company: [],
          load_destination_type: [],
          load_destination_address_1: [],
    ...
    mounted() {
      this.loadLoadInfo()
        .then(() => {
          this.onLoadInfoLoaded();
        });

      if (this.user && !['Client', 'Driver'].includes(this.user.role)) {
        this.loadDrivers();
      }
    ...
    methods: {
      parseLoadData() {
        // Load shipper
        this.formData.driver_id = this.load.driver ? this.load.driver.id : null;
        this.formData.status = this.load.status;

        this.currentAttachments = [...this.load.attachments];

        this.formData.custom_load_number = this.load.custom_load_number;
        this.formData.description = this.load.shipper.description;
        this.formData.first_name = this.load.shipper.first_name;
        this.formData.last_name = this.load.shipper.last_name;
        this.formData.email = this.load.shipper.email;
        this.formData.phone = this.load.shipper.phone_masked;
        this.formData.type = this.load.shipper.type;
        this.formData.company = this.load.shipper.company;
        this.formData.address_1 = this.load.shipper.address_1;
        this.formData.address_2 = this.load.shipper.address_2;
        this.formData.mobile = this.load.shipper.mobile_masked;
        this.formData.zip = this.load.shipper.zip;
        this.formData.city = this.load.shipper.city;
        this.formData.state = this.load.shipper.state;
        this.formData.phone_2 = this.load.shipper.phone_2_masked;
        this.formData.fax = this.load.shipper.fax;
        this.formData.notes_from_shipper = this.load.shipper.notes_from_shipper;

        // Load origin
        this.formData.load_origin_name = this.load.origin.name;
        this.formData.load_origin_zip = this.load.origin.zip;
        this.formData.load_origin_state = this.load.origin.state;
        this.formData.load_origin_city = this.load.origin.city;
        this.formData.load_origin_address_1 = this.load.origin.address_1;
        this.formData.load_origin_address_2 = this.load.origin.address_2;

        this.formData.load_origin_type = this.load.origin.type;
        this.formData.load_origin_company = this.load.origin.company;
        this.formData.load_origin_phone = this.load.origin.phone_masked;
        this.formData.load_origin_phone_2 = this.load.origin.phone_2_masked;
        this.formData.load_origin_mobile = this.load.origin.mobile_masked;
        this.formData.load_origin_fax = this.load.origin.fax;
        this.formData.load_origin_email = this.load.origin.email;

        // Load destination
        this.formData.load_destination_name = this.load.destination.name;
        this.formData.load_destination_zip = this.load.destination.zip;
        this.formData.load_destination_state = this.load.destination.state;
        this.formData.load_destination_city = this.load.destination.city;
        this.formData.load_destination_address_1 = this.load.destination.address_1;
        this.formData.load_destination_address_2 = this.load.destination.address_2;

        this.formData.load_destination_type = this.load.destination.type;
        this.formData.load_destination_company = this.load.destination.company;
        this.formData.load_destination_phone = this.load.destination.phone_masked;
        this.formData.load_destination_phone_2 = this.load.destination.phone_2_masked;
        this.formData.load_destination_mobile = this.load.destination.mobile_masked;
        this.formData.load_destination_fax = this.load.destination.fax;
        this.formData.load_destination_email = this.load.destination.email;

        // Load transport
        this.formData.available_date = this.load.transport.available_date;
        this.formData.delivery_date = this.load.transport.delivery_date;

        this.formData.available_accuracy = this.load.transport.available_accuracy;
        this.formData.delivery_accuracy = this.load.transport.delivery_accuracy;

        this.formData.distance = this.load.transport.distance;
        this.formData.price = this.load.transport.price;
        this.formData.deposit = this.load.transport.deposit;
        this.formData.carrier_type = this.load.transport.carrier_type;

        this.formData.carrier_pay = this.load.transport.carrier_pay;
        this.formData.carrier_pay_terms = this.load.transport.carrier_pay_terms;
        this.formData.customer_pay_terms = this.load.transport.customer_pay_terms;

        // Load vehicles
        this.formData.load_vehicles = [];
        this.load.vehicles.forEach(vehicle => {
          this.formData.load_vehicles.push({...vehicle});
        })
      },
      loadLoadInfo() {
      ...
      onLoadInfoLoaded() {
        ...
        if($('.datePicker').length) {
          ...
          $('#available_date_datepicker').datepicker('setDate', new Date(this.load.transport.available_date));
          $('#delivery_date_datepicker').datepicker('setDate', new Date(this.load.transport.delivery_date));
      ...
      loadStatuses() {
        return client.get('/api/load-statuses-data')
          .then(res => {
      ...
      updateLoadRequest() {
        ...
        client.post('/api/shipper/' + this.$route.params.shipper_id + '/loads/' + this.$route.params.load_id, fd, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        })
          .then(res => {
            this.updatingLoad = false;

            this.$swal('Success', 'Load has been updated', 'success')
              .then(() => {
                router.push(
                  {
      ...
      updateLoad() {
        if (this.updatingLoad) return;
        this.updatingLoad = true;
        this.clearCreateErrors();

        this.$swal.fire({
          title: 'Update Load',
          text: 'Your Load is being updating',
          onOpen: () => {
